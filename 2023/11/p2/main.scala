...
object Main extends IOApp {
  ...
  def solve1(input: ProblemState0): ProblemState1 = {
    val rows = input.grid
      .map(x => if (x.exists(_ == '#')) 0L else SPACE_SIZE)
      .foldLeft((0L, List.empty[Long])) { case ((sum, acc), x) =>
        (sum + x, acc :+ sum)
      }
      ._2
  ...
  def solve3(input: ProblemState2): ProblemState3 = {
    ProblemState3(input.galaxies.map { galaxy =>
      Galaxy(
        galaxy.x + input.rows(galaxy.x.toInt),
        galaxy.y + input.cols(galaxy.y.toInt)
      )
    })
  }

  def solve4(input: ProblemState3): Long = {
    input.galaxies.combinations(2).foldLeft(0L) { (sum, pair) =>
