...
fn main() {
    ...
    for distance in distances.iter().take(connections) {
        disjoint_set.union(distance.lhs, distance.rhs);
    }
    disjoint_set.rank.sort();
    disjoint_set.rank.reverse();
    let result: usize = disjoint_set.rank.iter().take(3).product();
    println!("{}", result);
}
